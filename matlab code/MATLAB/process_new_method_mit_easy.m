function [out_x_coord1, out_x_coord2, out_y_final1_eff, out_y_final2_eff] = process_new_method_mit_easy(in_size_states, in_e_samples, in_num_sim)


base_filename = 'variants_mit_';
% controlled var
size_actions = 100;
% # of total considered events
% the number of all considered states are (0) - (X), where X >= 1, all
% states not equal to 0 are interesting states, i.e., new contacts appear
% state transition matrix is thus X+1 times X+1, size_states = X+1
size_states = in_size_states;
...
for num_sim = 1:num_simulations
    ...
    %         ER_uni = 0;
    % 24 data files
    for data_file = [6, 7, 8, 11, 12, 14, 17, 18, 19, 20, 21, 22, 23, 24, 28, 29, 30, 31, 33, 35, 37, 39, 42, 46]
        ...
        for e_budget = e_precision:e_precision:1
            ...
            % get the # of events captured by adaptive
            ...
            while i <= num_samples
                ...
                if cur_event >= size_states - 1
                    cur_event_state = size_states;
                else
                    cur_event_state = cur_event + 1;
                end
        ...
        %disp(states_share);
        % with two states
        %plot(x_coord1, states_share(1)*y_coord1 + states_share(2)*y_coord2, 'b--o', x_coord, y_coord, 'r');
        % with three states
        %plot(x_coord1, states_share(1)*y_coord1 + states_share(2)*y_coord2 + states_share(3)*y_coord3, 'b--o', x_coord, y_coord, 'r');
        % with four states
        ...
        %plot(x_coord1, y_coord1, 'b--o', x_coord2, y_coord2, 'g--o', x_coord3, y_coord3, 'b.', x_coord4, y_coord4, 'r', x_coord, y_coord, 'y--o');
        y_coord_action = y_coord_action + states_share(1)*y_coord1 + states_share(2)*y_coord2;
